...
def SPCA_cv(param_spca, return_variance):
    '''
    This function performs supervised PCA estimates of risk premium with cross-validation.
    
    Input:
    param_spca.rt: n by T matrix
    param_spca.gt: d by T factor proxies
    param_spca.usep: # of factors we use (1 by J vector)
    param_spca.tuning: # of test assets we use at each step
    param_spca.q: (optional) parameter, default is 1

    Output:
    res.Gammahat_nozero: d by J vector of risk premia estimates
    res.b: J by N vector of SDF loading
    res.mimi: d by N by J weights matrix for mimicking portfolio
    res.avarhat_nozero: d by J vector of variance estimator for rp
    res.alphahat: d by J vector of pricing errors
    res.avarhat_alpha: d by J vector of variance estimator for alpha
    '''
    # INITIALIZATION
    param = param_spca
    if 'q' not in param:
    ...
    n = rt.shape[0]
    d = gt.shape[0]
    if isinstance(usep, (int, np.integer)):
        usep = np.array([usep])
    J = len(usep)
    pmax = np.max(usep)
    
    # ESTIMATION
    rtbar = rt - np.tile(np.mean(rt, axis=1, keepdims=True),(1,T))
    gtbar = gt - np.tile(np.mean(gt, axis=1, keepdims=True), (1,T))
    ...
    while  kk < pmax:
        COR = np.abs(np.corrcoef(np.hstack((rt_0.T, gt0.T)), rowvar=False)\
            [:-gt0.shape[0], -gt0.shape[0]:])
        L = np.max(COR, axis=1) 
        bb = np.sort(L)
        if N0 < n:
            II = (L >= bb[n-N0])
            ...
            II = L > -1
        kk += 1
        U, s, Vt = svds(rt_0[II, :], k=1, which='LM',solver='arpack')
        V = Vt.T 
        B[kk-1, II] = U[:, 0].T / s[0]
        gammahat_all[kk-1, :] = U[:, 0].T @ mrt_0[II] / s[0]
        etahat_all[:, kk-1] = gt0 @ V[:, 0]

        # Projection
        gt0 = gt0 - gt0 @ V[:, 0].reshape(-1,1) @ V[:, 0].reshape(-1,1).T
        mrt_0 = mrt_0 - rt_0 @ V[:, 0].reshape(-1,1) @ gammahat_all[kk-1, :].reshape(-1,1) 
        rt_0 = rt_0 - rt_0 @ V[:, 0].reshape(-1,1) @ V[:, 0].reshape(-1,1).T
        vhat_all[kk-1, :] = V[:, 0].T
        
    for jj in range(len(usep)):
        phat = usep[jj]
        vhat = vhat_all[:phat, :]
        ...
        if return_variance:
            ...
            for t in range(T):
                Pi11hat += np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T)) / T
                Pi12hat += np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), vhat[:, t]) / T
                Pi22hat += np.vstack(vhat[:, t]) @ np.vstack(vhat[:, t]).T / T
                Pi13hat += np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), np.vstack(what[:, t])) / T
                Pi33hat += np.vstack(what[:, t]) @ np.vstack(what[:, t]).T / T

                for s in range(min(t, q)):
                    Pi11hat += 1 / T * (1 - s / (q + 1)) * (
                        np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), np.ravel(np.vstack(what[:, t - s]) @ np.vstack(vhat[:, t - s]).T)) +
                        np.outer(np.ravel(np.vstack(what[:, t - s]) @ np.vstack(vhat[:, t - s]).T), np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T))
                        )        
                    Pi12hat += 1 / T * (1 - s / (q + 1)) * (
                        np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), np.vstack(vhat[:, t - s]).T) +
                        np.outer(np.ravel(np.vstack(what[:, t - s]) @ np.vstack(vhat[:, t - s]).T), np.vstack(vhat[:, t]).T)
                        )                        
                    Pi22hat += 1 / T * (1 - s / (q + 1)) * (
                        np.vstack(vhat[:, t]) @ np.vstack(vhat[:, t - s]).T + np.vstack(vhat[:, t - s]) @ np.vstack(vhat[:, t]).T
                        )
                    Pi13hat += 1 / T * (1 - s / (q + 1)) * (
                        np.outer(np.ravel(np.vstack(what[:, t]) @ np.vstack(vhat[:, t]).T), what[:, t - s].T) +
                        np.outer(np.ravel(np.vstack(what[:, t - s]) @ np.vstack(vhat[:, t - s]).T), np.vstack(what[:, t]).T)
                        )
                    Pi33hat += 1 / T * (1 - s / (q + 1)) * (
                        np.vstack(what[:, t]) @ np.vstack(what[:, t - s]).T +
                        np.vstack(what[:, t - s]) @ np.vstack(what[:, t]).T 
                        ) 
                avarhat_nozero[:, jj] = np.diag(np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), \
                    np.eye(d)) @ Pi11hat @ np.kron(np.linalg.pinv(Sigmavhat) @ gammahat, np.eye(d))/T + \
                        np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), np.eye(d)) @ Pi12hat @ etahat.T/T + \
                            (np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), np.eye(d)) @ Pi12hat @ etahat.T).T/T + \
                                etahat @ Pi22hat @ etahat.T/T)

        # RISK PREMIA ESTIMATION
        Gammahat_nozero[:, jj] = np.ravel(etahat @ gammahat)
        
        B_sub = B[:phat, :]
        fhat = B_sub @ rt
        fhatbar = fhat - np.mean(fhat, axis=1, keepdims=True)
        mimi[:, :, jj] = (gt - np.mean(gt, axis=1, keepdims=True)) @ fhatbar.T @ np.linalg.pinv(fhatbar @ fhatbar.T) @ B_sub
        b[jj, :] = np.mean(fhat, axis=1).T @ np.linalg.pinv(fhatbar @ fhatbar.T/T) @ B_sub
        
        # ESTIMATE alpha = E(g) - eta*gamma
        alphahat[:, jj] = np.mean(gt, axis=1) - Gammahat_nozero[:, jj]
        if return_variance:
            avarhat_alpha[:, jj] = np.diag(np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), np.eye(d)) @ Pi11hat @ \
                np.kron(np.linalg.pinv(Sigmavhat) @ gammahat, np.eye(d))/T -\
                    np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), np.eye(d)) @ Pi13hat/T - \
                        (np.kron(gammahat.T @ np.linalg.pinv(Sigmavhat), np.eye(d)) @ Pi13hat).T/T + 
                                    Pi33hat/T)
    # RESULTS
    res = {
